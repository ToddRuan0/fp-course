...
instance Functor Parser where
  ...
  (<$>) f pa =
    (pure . f) =<< pa
    -- don't use (>>=) here, because Course.Monad use (<$>) to implement (>>=)
    -- pa >>= \a -> valueParser (f a)
...
(|||) ::
  ...
  -> Parser a
(|||) pa pb =
  P $ \i -> let res = parse pa i in
    ifThenElse (isErrorResult res) (parse pb i) res
...
instance Monad Parser where
  (=<<) ::
    ...
    -> Parser b
  (=<<) f pa =
    P $ \i -> onResult (parse pa i) (\i' a -> parse (f a) i')
    -- P $ \i -> onResult (f <$> parse pa i) (flip parse)
     -- case f <$> (parse pa i) of
     --  Result i2 pb -> parse pb i2
     --  _ -> UnexpectedEof`  2222221
...
satisfy ::
  ...
  -> Parser Char
satisfy f =
  character >>= \a -> ifThenElse (f a) (valueParser a) (unexpectedCharParser a)
...
digit =
  satisfy isDigit
  -- character >>= \a -> ifThenElse (elem a $ listh "0123456789") (valueParser a) (unexpectedCharParser a)
--  P $ \i -> case i of
--     x :. xs -> ifThenElse (elem x $ listh "0123456789") (Result xs x) (UnexpectedString xs)
--     _ -> UnexpectedEof
...
phoneParser ::
  Parser Chars
phoneParser =
  (\a b _ -> a :. b) <$> digit <*> phoneBodyParser <*> (is '#')
