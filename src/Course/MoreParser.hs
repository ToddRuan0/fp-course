...
betweenCharTok ::
  ...
  -> Parser a
  -> Parser a
betweenCharTok b e pa =
  between (charTok b) (charTok e) pa
...
sure :: Monad f => f a -> f (Optional a) -> f a
sure fa foa = foa >>= \oa -> case oa of
  Full a -> pure a
  Empty -> fa
...
hex ::
  Parser Char
hex =
  let hexP = satisfy isHexDigit
      x = replicateA 4 hexP in
      x >>= \cs -> sure (P $ \_ -> UnexpectedString cs) (pure $ chr <$> readHex cs)
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma b e pa =
  betweenCharTok b e $ sepby pa (charTok ',')
